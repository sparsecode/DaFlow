...
class SQLParser extends JavaTokenParsers {
  ...
  def sqlLiterals: SQLParser.this.Parser[String] = stringLiteral | stringLiteral1

  def cmp_expr:Parser[String] = "=" | ">=" | "<=" | "<>" | "!=" | ">" | "<"
  def boolean: SQLParser.this.Parser[Boolean] = "true" ^^^ "true".toBoolean | "false" ^^^ "false".toBoolean

  ...
  def selectColumn(row: Row, dataType: DataType, colName: String): Either[Any, String] = {
    ...
    try {
      value = dataType match {
          ...
          case p: org.apache.spark.sql.types.StructType =>
            ...
            val temp: Try[Seq[Any]] = Try(arrayData.map(row => {
              val s: Array[Any] = structFields.map(field => {
                ...
                else { Logger.log.error("Array Type inner error: " + f.right.get); null}
              })
        ...
        case default => Logger.log.error(s"column data type is not supported: $default")
      ...
      case NonFatal(e) => Logger.log.debug(s"column projection $colName, exception: ${e.getMessage}, ${e.getClass}")
    }
    Left(value)
  }

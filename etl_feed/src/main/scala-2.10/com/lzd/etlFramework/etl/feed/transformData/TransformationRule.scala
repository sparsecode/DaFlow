...
import com.lzd.etlFramework.etl.feed.common.sql_parser.{Clause, SQLParser}
import com.lzd.etlFramework.etl.feed.common.{Context, FieldMapping, Load}
...
class PartitionRule(order: Int, scope: String, ruleCondition: String, group: Int)
  ...
  private def setCondition(conditions: Seq[Clause]) = {
    conditions.foreach {
      case notNullClause: com.lzd.etlFramework.etl.feed.common.sql_parser.NotNull =>
        condition = s"${notNullClause.getFields("field")} IS NOT NULL"
        notCondition = s"${notNullClause.getFields("field")} IS NULL"

      case nullClause: com.lzd.etlFramework.etl.feed.common.sql_parser.Null =>

...
class LoadDataIntoHive(feed: LoadFeedConf) extends LoadData {
  ...
  private val processFrequency = Context
  ...
  private val hiveTableDataInitialPath =
    FileUtil.getFilePathString(feed.dataPath)

  def loadTransformedData(
    dataFrame: DataFrame,
    date: Option[DateTime] =
      Context.getContextualObject[Option[DateTime]](START_DATE)
  ): Either[Boolean, String] = {
    val dateString = date.get.toString("yyyy-MM-dd")
    val timeString =
      s"""${new DecimalFormat("00").format(date.get.getHourOfDay)}"""
    val path =
      s"$hiveTableDataInitialPath/$databaseName/$tableName/$dateString/$timeString"

    try {

      logger.info(
        s"Writing $processFrequency dataFrame for table $tableName to HDFS ($path). " +
      ...
      val df: DataFrame = if (partData.coalesce) {
        ...
        val numExecutors =
          sqlContext.sparkContext.getConf.get("spark.executor.instances").toInt
        if (currentPartitions > numExecutors) {
          dataFrame.coalesce(partData.coalesceCount)
        } else { dataFrame }
      } else { dataFrame }
  ...
  private def writeHiveData(path: String): Unit = {
    ...
    logger.info(
      s"Partition at ($path) registered successfully to $databaseName.$tableName"
    )
  }
}

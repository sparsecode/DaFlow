...
class LoadDataIntoFileSystem(feed: LoadFeedConf) extends LoadData {
  ...
  private val datasetName: String =
    feed.attributesMap.getOrElse("catalogName", "")
  ...
  def loadTransformedData(
    ...
    try {
      logger.info(
        s"Writing $processFrequency dataFrame for dataset: $datasetName, feed $feedName to ($path). " +
      ...
      val output: Either[Boolean, String] = fileType match {
        case "CSV" =>
          dataFrame.write.mode(SaveMode.Overwrite).csv(path)
